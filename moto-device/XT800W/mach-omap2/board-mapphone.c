...
#define MAPPHONE_TOUCH_RESET_N_GPIO	13
#define MAPPHONE_TOUCH_INT_GPIO		99
#define MAPPHONE_LM_3530_INT_GPIO	100
#define MAPPHONE_AKM8973_INT_GPIO	175
#define MAPPHONE_AUDIO_PATH_GPIO	143
#define MAPPHONE_BP_READY_AP_GPIO	19
#define MAPPHONE_BP_READY2_AP_GPIO	114
#define MAPPHONE_BP_RESOUT_GPIO		10
#define MAPPHONE_BP_PWRON_GPIO		35
#define MAPPHONE_AP_TO_BP_PSHOLD_GPIO	40
#define MAPPHONE_AP_TO_BP_FLASH_EN_GPIO	20
#define MAPPHONE_POWER_OFF_GPIO		176
#define MAPPHONE_APWAKE_TRIGGER_GPIO	19
#else /* CONFIG_TTU */
...
#define MAPPHONE_LM_3530_RESET_GPIO	22
#define MAPPHONE_AKM8973_INT_GPIO	175
#define MAPPHONE_AUDIO_PATH_GPIO	143
...
static void mapphone_als_init(void)
{
	...
	if (lm3530_reset_gpio < 0) {
		printk(KERN_DEBUG"mapphone_als_init: cann't get lm3530_reset from device_tree\n");
		lm3530_int_gpio = MAPPHONE_LM_3530_RESET_GPIO;
	}
...
/* A dummy HW touch key */
static struct qtouch_key  dummy_ts_key[] = {
	{
		.channel	= 31,
		.code		= 0,
	},
...
static struct qtouch_ts_platform_data mapphone_ts_platform_data = {
	...
	.fw_version	= 0x16,
	.build_version	= 0xab,
	.base_fw_version	= 0x16, /* dummy for TTSKT */
	.fuzz_x			= 0,
	.fuzz_y			= 0,
	.fuzz_p			= 2,
	.fuzz_w			= 2,
	...
	.self_test_cfg = {
		.ctrl			= 0,
		.command		= 0,
		.high_signal_limit_0    = 9999,
		.low_signal_limit_0     = 1000,
...
)
{
	...
	static char *i2c_devices[] = {
			"qtouch-obp-ts,lm3530_led,airc,lp3907,tda19989",
...
static void __init mapphone_serial_init(void)
{
	...
	omap_serial_init(get_gpio_by_name("ipc_o_bp_flash_en"), 0x01 << 2);
}
...
static void __init mapphone_vout_init(void)
{
	...
	platform_data->max_width = 1280;
	platform_data->max_height = 720;
	platform_data->max_buffer_size = 0x1C3000;
#endif
...
#ifdef CONFIG_TTU
static void __init mapphone_gps_en_init(void)
{
	/* config gpio23 into GPS mode with High status */
	int gps_en, ret;

	gps_en = get_gpio_by_name("gps_en");

	if (gpio_request(gps_en, "GPS EN") != 0) {
		printk(KERN_ERR "gpio_request is failed about gps_en.\n");
		return;
	}

	gpio_direction_output(gps_en, 1);
